...
export async function createStripeCheckoutSession(
  bookingId: number,
  domainUrl: string, // Ce paramètre pourrait devenir superflu si NEXT_PUBLIC_BASE_URL est toujours utilisé.
  stripeCustomerId: string
): Promise<string> {
  try {
    const booking = await prisma.booking.findUnique({
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      // --- MODIFICATION ICI ---
      // On retire booking_id de l'URL de succès. Le webhook a déjà bookingId via les métadonnées.
      // La page de succès n'aura besoin que de session_id pour demander un token.
      success_url: `${process.env.NEXT_PUBLIC_BASE_URL}/payment/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL}/payment/cancel`,
      // --- FIN MODIFICATION ---
      metadata: {
