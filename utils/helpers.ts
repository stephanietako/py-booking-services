...
import {
  ...
  isEqual,
  format,
  isWithinInterval, // Importez isWithinInterval
} from "date-fns";
import { fr } from "date-fns/locale";
import { DayInput } from "@/types";
import {
...
export const filterAvailableTimes = (
  availableTimes: Date[],
  bookedTimes: { start: Date; end: Date }[] // Mettez à jour le type de bookedTimes
): Date[] => {
  return availableTimes.filter((availableTime) => {
    // Vérifier si l'heure de début potentielle chevauche une réservation existante
    const overlapsWithExisting = bookedTimes.some((bookedSlot) =>
      isWithinInterval(availableTime, {
        start: bookedSlot.start,
        end: bookedSlot.end,
      })
    );
    return !overlapsWithExisting;
  });
};
