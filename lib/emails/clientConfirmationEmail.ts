...
//   Option,
//   Service,
// } from "@prisma/client"; // Import Service type from Prisma
...
//   const formattedStart = format(new Date(startTime), "dd/MM/yyyy 'à' HH:mm");
...
//   const subject = `Confirmation de votre réservation - ${serviceName} le ${format(new Date(startTime), "dd/MM/yyyy")}`;
...
//     ${
//       booking.comment
//         ? `<p style="margin-top:18px;"><strong>Commentaire client :</strong> ${escapeHtml(booking.comment)}</p>`
//         : ""
//     }
//     ${optionsHtml}

//     <p style="font-size: 17px; font-weight: bold; color: #005ea2; margin-top: 25px;">
//         Total à régler à bord (options + capitaine) : ${formatter.format(totalPayableOnBoardCalculated)}
//     </p>
...
//     `Montant de la location du bateau réglé en ligne : ${formatter.format(boatAmount)}\n` +
//     `Capitaine : ${captainText}\n` +
//     `Option repas traiteur : ${mealOption ? "Oui. L'administrateur vous contactera pour les détails et prix." : "Non."}\n` +
...
//     `Téléphone : ${phoneNumber}\n\n` +
//     `Nous avons hâte de vous accueillir à bord et vous souhaitons une excellente expérience !\n\n` +
//     (booking.comment ? `Commentaire client : ${booking.comment}\n` : "") +
//     `Cordialement,\n` +
//     `L’équipe Yachting Day`;
...
//   // Sécurisation de la date : fallback si invalide
...
//   const serviceName = escapeHtml(service?.name || "Service Réservé");
...
//       const qty = opt.quantity ?? 1;
//       const pricePerUnit = opt.option?.unitPrice ?? 0;
//       const name = escapeHtml(opt.option?.label ?? "Option inconnue");
//       return `<li>${qty} × ${name} — ${formatter.format(pricePerUnit * qty)}</li>`;
//     })
//     .join("");
...
//   const logoUrl =
//     process.env.LOGO_URL || "https://yachtingday.com/assets/logo/logo-new.png"; // Remplace par ta vraie URL de prod
...
//   <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; background-color: #f9f9f9;">
//     <div style="text-align: center; margin-bottom: 30px;">
//       <img src="${logoUrl}" alt="Logo Yachting Day - Votre expérience en mer" width="180" style="max-width: 100%; height: auto;">
//     </div>
//     <h2 style="color: #005ea2; text-align: center; margin-bottom: 25px;">Bonjour ${escapeHtml(
//       fullName
//     )},</h2>
//     <p style="font-size: 16px;">
//       Merci pour votre réservation de <strong style="color: #007bff;">${serviceName}</strong> avec <strong style="color: #005ea2;">Yachting Day</strong>.
...
// Fonction utilitaire pour convertir proprement une date, sans utiliser any
