...
// Fonction safeDate améliorée avec logging et validation
function safeDate(
  ...
  if (date instanceof Date) {
    if (isNaN(date.getTime())) {
      console.error(`Date invalide (NaN) dans le contexte: ${context}`, date);
      return new Date();
    }
    return date;
  }
  ...
  if (typeof date === "string") {
    ...
    console.error(
      `Impossible de parser la date string dans le contexte: ${context}`,
      date
    );
    return new Date();
  }
  ...
  console.error(
    `Type de date non supporté dans le contexte: ${context}`,
    typeof date,
    date
  );
  return new Date();
...
// Fonction pour formater intelligemment la période de réservation
function formatReservationPeriod(
  ...
  // Validation que endTime > startTime
  if (end <= start) {
    console.warn(
      "Attention: La date de fin n'est pas postérieure à la date de début",
      {
  ...
  const endFormatted = isSameDay
    ...
    : format(end, "dd/MM/yyyy 'à' HH:mm", { locale: fr });

  return `Du ${startFormatted} au ${endFormatted}`;
}

// Fonction pour formater une date simple de manière sécurisée
function formatDateSafe(
  ...
  try {
    const safeDateTime = safeDate(date, context);
    return format(safeDateTime, formatString, { locale: fr });
  } catch (error) {
    console.error(
      `Erreur lors du formatage de la date dans le contexte: ${context}`,
      error
    );
...
export function buildClientConfirmationEmail(
  ...
  // Validation des dates critiques
  if (!startTime || !endTime) {
    console.error("Dates manquantes pour la réservation", {
      startTime,
      endTime,
    });
  }
  ...
  const reservationPeriod = formatReservationPeriod(startTime, endTime);
  const subjectDate = formatDateSafe(startTime, "dd/MM/yyyy", "subject date");
  const serviceName = service?.name || "Service Réservé";

  // Vérification optionnelle si la date est dans le passé
  const startDate = safeDate(startTime, "validation start");
  if (startDate < new Date()) {
    console.warn("Attention: La réservation est dans le passé", {
  ...
  const optionsHtmlList = bookingOptions
    ...
    .map((opt) => {
      ...
      const pricePerUnit = opt.option?.unitPrice ?? 0;
      const name = opt.option?.label ?? "Option inconnue";
      return `<li>${qty} × ${escapeHtml(name)} — ${formatter.format(pricePerUnit * qty)}</li>`;
    })
    .filter(Boolean)
    .join("");
  ...
  const subject = `Confirmation de votre réservation - ${serviceName} le ${subjectDate}`;
  ...
  <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; background-color: #f9f9f9;">
    ...
    <h2 style="color: #005ea2; text-align: center; margin-bottom: 25px;">Bonjour ${escapeHtml(fullName)},</h2>
    <p style="font-size: 16px;">
      Merci pour votre réservation de <strong style="color: #007bff;">${escapeHtml(serviceName)}</strong> avec <strong style="color: #005ea2;">Yachting Day</strong>.
    </p>
    <p style="font-size: 16px;">
      Votre réservation a été confirmée ! Voici les détails :
    </p>
    <ul style="list-style-type: none; padding: 0; margin-bottom: 25px;">
      <li><strong>Date et heure :</strong> ${reservationPeriod}</li>
      <li><strong>Service réservé :</strong> ${escapeHtml(serviceName)}</li>
      <li><strong>Montant de la location du bateau réglé en ligne :</strong> <span style="color: #28a745; font-weight: bold;">${formatter.format(
        boatAmount
      )}</span></li>
      <li><strong>Capitaine :</strong> ${captainText}</li>
    ...
    <p style="font-size: 16px;">
      ...
      Téléphone : ${escapeHtml(phoneNumber)}
    </p>
    <p style="font-size: 16px;">
      Nous avons hâte de vous accueillir à bord et vous souhaitons une excellente expérience !
    </p>
...
`;
  ...
  const text =
    ...
    `Merci pour votre réservation de ${serviceName} avec Yachting Day.\n\n` +
    `Votre réservation a été confirmée ! Voici les détails :\n` +
    `Date et heure : ${reservationPeriod}\n` +
    `Service réservé : ${serviceName}\n` +
    `Montant de la location du bateau réglé en ligne : ${formatter.format(
      boatAmount
    )}\n` +
