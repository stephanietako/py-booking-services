...
const AdminUserClientList = () => {
  ...
  // Filtrer les clients pour exclure ceux qui ont un email déjà présent chez les users (éviter doublons)
  const userEmails = new Set(users.map((user) => user.email.toLowerCase()));
  const filteredClients = clients.filter(
    (client) => !userEmails.has(client.email.toLowerCase())
  );
    ...
    <div className={styles.container}>
      ...
      {loading ? (
        ...
        <>
          ...
          <Table
            data={filter(filteredClients).map((client) => ({
              id: client.id,
