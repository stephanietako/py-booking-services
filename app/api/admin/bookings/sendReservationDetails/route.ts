...
import { sendEmail } from "@/lib/email/send";
import { PrismaClient } from "@prisma/client";
import { buildAdminReservationEmail } from "@/lib/emails/adminReservationDetails"; // Importe AdminReservationEmailParams implicitely via buildAdminReservationEmail
...
function getContactInfo(booking: {
  ...
  if (booking.client && booking.client.fullName && booking.client.email) {
    ...
    // Fallback pour assurer que les retours sont toujours du même type
    return {
      fullName: "",
      email: "",
      phoneNumber: "",
  ...
  try {
    ...
    const booking = await prisma.booking.findUnique({
      ...
      include: {
        ...
        user: true,
        service: {
          include: { pricingRules: true }, // Assurez-vous que pricingRules est nécessaire ou retiré si non utilisé
        },
        bookingOptions: {
          include: { option: true },
        },
    ...
    // Le commentaire ci-dessous est valide si pricingRules est toujours attendu par la suite
    if (!booking.service.pricingRules) {
      throw new Error("Aucune règle de prix trouvée pour ce service !");
    }
    ...
    // --- Paramètres pour l'e-mail client (aucune modification nécessaire ici, ils sont déjà corrects) ---
    const clientEmailParams = {
      bookingId: booking.id.toString(),
      clientName: contactInfo.fullName,
      clientEmail: contactInfo.email,
      clientPhoneNumber: contactInfo.phoneNumber,
      serviceName: booking.service.name || "",
      startTime: booking.startTime, // C'est déjà correct
      endTime: booking.endTime, // C'est déjà correct
      boatAmount,
      mealOption: booking.mealOption ?? null,
      withCaptain: booking.withCaptain,
      captainPrice,
    ...
    // --- Paramètres pour l'e-mail admin (CORRECTION APPORTÉE ICI) ---
    const emailParams = {
      bookingId: booking.id.toString(),
      firstName: contactInfo.fullName.split(" ")[0] || "",
      lastName: contactInfo.fullName.split(" ").slice(1).join(" ") || "",
      email: contactInfo.email,
      phoneNumber: contactInfo.phoneNumber,
      // CORRECTION : AJOUT DES PROPRIÉTÉS startTime et endTime
      startTime: booking.startTime, // Assurez-vous que booking.startTime est bien un Date object ou un string ISO valide
      endTime: booking.endTime, // Assurez-vous que booking.endTime est bien un Date object ou un string ISO valide
      reservationTime: new Date(booking.startTime).toLocaleString("fr-FR"), // Cette propriété est différente et non requise par AdminReservationEmailParams
      stripeUrl,
      comment: booking.description || "",
      bookingOptions: (booking.bookingOptions ?? []).map((bo) => ({
        quantity: bo.quantity,
    ...
    // Envoi email admin
    // emailParams contient maintenant startTime et endTime
    const { subject, html, text } = buildAdminReservationEmail(emailParams);
    await sendEmail({ to: adminEmail, subject, html, text });
