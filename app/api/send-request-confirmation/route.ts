...
import { sendEmail } from "@/lib/email/send";
import { PrismaClient } from "@prisma/client";
import type { Service, PricingRule } from "@/types"; // Assurez-vous que ces types sont correctement définis
...
export async function POST(request: Request) {
  try {
    ...
    // Paramètres pour l'e-mail client (aucune modification nécessaire ici)
    const clientEmailParams = {
      bookingId: booking.id.toString(),
      clientName: contactInfo.fullName,
      clientEmail: contactInfo.email,
    ...
    const emailParams = {
      ...
      email: contactInfo.email,
      phoneNumber: contactInfo.phoneNumber,
      // AJOUTEZ CES DEUX LIGNES POUR CORRIGER L'ERREUR DE TYPAGE :
      startTime: booking.startTime, // La propriété 'startTime' est requise
      endTime: booking.endTime, // La propriété 'endTime' est requise
      reservationTime: new Date(booking.startTime).toLocaleString("fr-FR"), // Cette ligne est correcte
      stripeUrl,
      comment: booking.description || "",
      bookingOptions: booking.bookingOptions.map((bo) => ({
        quantity: bo.quantity,
