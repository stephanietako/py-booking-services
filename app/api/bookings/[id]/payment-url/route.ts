...
export async function GET(req: NextRequest) {
  try {
    ...
    // ✅ Mise à jour de la base avec l'URL et l'ID de session
    await prisma.booking.update({
      where: { id: booking.id },
      data: {
        stripeSessionId: session.id,
        stripePaymentLink: session.url,
        paymentStatus: "PENDING", // optionnel : si tu veux gérer un statut initial
      },
    });
...
// Enregistrer stripeSessionId et stripePaymentLink dans la base.

// Laisser paymentStatus à PENDING jusqu’au retour du webhook.
